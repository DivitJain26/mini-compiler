...
const TYPE_BADGE: Record<TokenType, string> = {
  KEYWORD:       "bg-sky-900/60 text-sky-300 border-sky-700",
  IDENTIFIER:    "bg-slate-700/60 text-slate-200 border-slate-600",
  INT_LITERAL:   "bg-orange-900/60 text-orange-300 border-orange-700",
  FLOAT_LITERAL: "bg-amber-900/60 text-amber-300 border-amber-700",
  BOOL_LITERAL:  "bg-teal-900/60 text-teal-300 border-teal-700",
  OPERATOR:      "bg-rose-900/60 text-rose-300 border-rose-700",
  DELIMITER:     "bg-zinc-800/60 text-zinc-400 border-zinc-600",
  EOF:           "bg-zinc-800/60 text-zinc-500 border-zinc-700",
};
        ...
        <tbody>
          {tokens.map((tok, i) => (
            ...
            >
              ...
              <td className="px-4 py-2">
                <span
                  className={`inline-block px-2 py-0.5 rounded text-xs font-mono font-medium border ${
                    TYPE_BADGE[tok.type] ?? "bg-slate-700 text-slate-300 border-slate-600"
                  }`}
                >
                  {tok.type}
                </span>
              ...
              <td className="px-4 py-2 text-slate-500 tabular-nums">{tok.line}</td>
            </tr>
          ))}
        </tbody>
      </table>
