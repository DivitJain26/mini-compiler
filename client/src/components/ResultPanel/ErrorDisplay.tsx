...
export default function ErrorDisplay({ errors, wasCompiled }: ErrorDisplayProps) {
  ...
  if (errors.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-3">
        ...
        <p className="text-sm text-green-400">No errors found — compilation successful</p>
      </div>
    );
  }
  ...
  return (
    <div className="overflow-auto h-full p-4 space-y-3">
      {/* Summary bar */}
      <div className="flex items-center gap-4 px-4 py-2.5 rounded-lg bg-red-950/40 border border-red-900/50">
        ...
        <div className="flex gap-3 ml-auto text-xs">
          {lexicalErrors.length > 0 && (
            <span className="text-amber-400">{lexicalErrors.length} Lexical</span>
          )}
          {syntaxErrors.length > 0 && (
            <span className="text-rose-400">{syntaxErrors.length} Syntax</span>
          )}
        </div>
      </div>
        ...
        {errors.map((err, i) => (
          <div
            ...
            className={`flex items-start gap-3 p-3.5 rounded-lg border transition-all ${
              err.kind === "Lexical"
                ? "bg-amber-950/30 border-amber-900/40 hover:border-amber-800/50"
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p
                className={`text-sm font-mono ${
                  err.kind === "Lexical" ? "text-amber-200" : "text-red-200"
                }`}
              >
                {err.message}
              </p>
            ...
            {/* Line number pill */}
            <span className="shrink-0 flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-slate-800 border border-slate-700 text-slate-400 text-xs font-mono">
